...
function getDay(e) {
  ...
  rows.reverse(); // 新的在前
  var res = { status: 'ok', date: key, rows: rows, totals: totals, ver: 'v3-H欄比對' };
...
function rowDateKey_(row) {
  if (row[7] instanceof Date) return Utilities.formatDate(row[7], TZ, 'yyyy/M/d');
  return fmtDateVal_(row[1]);
}

// 取列的顯示時間 'HH:mm'：優先用 H 建立時間，沒有才退回 C 欄
function rowTime_(row) {
  if (row[7] instanceof Date) return Utilities.formatDate(row[7], TZ, 'HH:mm');
